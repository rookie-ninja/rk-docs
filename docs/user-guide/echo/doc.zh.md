...
| 名字                   | 描述                                       | 类型       | 默认值   |
|----------------------|------------------------------------------|----------|-------|
| echo.docs.enabled     | 启动 API Docs                              | boolean  | false |
| echo.docs.path        | API Docs Web 界面路径                        | string   | docs  |
| echo.docs.specPath    | 本地 Swagger 参数文件（swagger.json）路径          | string   | ""    |
| echo.docs.headers     | 每次 Swagger 界面请求，都会带着这些头部。格式： [key:value] | []string | []    |
| echo.docs.style.theme | Web UI Theme，目前只支持 light                 | string   | light |
| echo.docs.debug       | 开启 Debug 模式，像 Swagger UI 一样向后台发送请求       | bool     | false |
