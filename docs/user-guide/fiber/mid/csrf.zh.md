...
| 名字                                   | 描述                                               | 类型 | 默认值 |
|--------------------------------------|--------------------------------------------------| ------ | ------ |
| fiber.middleware.csrf.enabled          | 启动 CSRF 中间件                                      | boolean | false |
| fiber.middleware.csrf.ignore           | 局部选项，忽略 API 路径                               | []string | []                   |
| fiber.middleware.csrf.tokenLength    | 生成的 CSRF Token 的长度                               | int | 32 |
| fiber.middleware.csrf.tokenLookup    | 寻找 CSRF Token 的方法，请参考代码注释                        | string | "header:X-CSRF-Token" |
| fiber.middleware.csrf.cookieName     | CSRF cookie 名字                                   | string | _csrf |
| fiber.middleware.csrf.cookieDomain   | CSRF cookie 的 Domain 名称                          | string | "" |
| fiber.middleware.csrf.cookiePath     | CSRF cookie 路径                                   | string | "" |
| fiber.middleware.csrf.cookieMaxAge   | CSRF cookie 的 MaxAge（秒）                          | int | 86400 |
| fiber.middleware.csrf.cookieHttpOnly | CSRF cookie 是否只支持 HTTP                           | bool | false |
| fiber.middleware.csrf.cookieSameSite | 描述 CSRF cookie 的 SameSite 模式。选项: lax, strict, none, default | string | default |
