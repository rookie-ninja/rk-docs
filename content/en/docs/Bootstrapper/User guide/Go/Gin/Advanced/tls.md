...
## Overview
In order to enable TLS on the server, we need server certificate and server key at least.

Bootstrapper will read ConfigEntry section in boot.yaml file and load certificates into memory.
...
> server.csr, server.pem and server-key.peom would be created.
```shell script
...
### 5.Access CertEntry
User can access ConfigEntry by calling **rkentry.GlobalAppCtx.GetCertEntry()**.
