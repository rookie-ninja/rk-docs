...
|-----------------------|-------------------------------------------------|----------|---------------|
| fiber.docs.enabled     | Enable API Docs                                 | boolean  | false         |
| fiber.docs.path        | API Docs Web path                               | string   | docs          |
| fiber.docs.specPath    | Local Swagger config file（swagger.json）path     | string   | ""            |
| fiber.docs.headers     | Headers returned by server, format: [key:value] | []string | []            |
| fiber.docs.style.theme | Web UI Theme，support light only currently       | string   | light         |
| fiber.docs.debug       | Enable Debug mode                               | bool     | false         |

## Quick start
### 1.Create boot.yaml
> rk-boot will search docs/, api/gen/v1/ folder for swagger JSON file
>
> If swagger JSON located at other location，**fiber.docs.jsonPath** needs to be configured
